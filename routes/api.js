...
router.get('/staff', apiAdmin, async (req, res) => {
  ...
  const counts = await Lead.findAll({ attributes: ['assigned_to', [sequelize.fn('COUNT', sequelize.col('id')), 'cnt']], where: { assigned_to: { [Op.ne]: null } }, group: ['assigned_to'], raw: true });
...
router.get('/leads', apiAuth, async (req, res) => {
  try {
    ...
    else if (assigned && assigned !== 'all') where.assigned_to = assigned;
    if (search) { where[Op.or] = [{ name: { [Op.like]: `%${search}%` } }, { phone: { [Op.like]: `%${search}%` } }, { email: { [Op.like]: `%${search}%` } }, { city: { [Op.like]: `%${search}%` } }, { category: { [Op.like]: `%${search}%` } }, { business_type: { [Op.like]: `%${search}%` } }]; }
    const offset = (parseInt(page) - 1) * parseInt(limit);
    const { count, rows } = await Lead.findAndCountAll({ where, limit: parseInt(limit), offset, include: [{ model: User, as: 'assignedStaff', attributes: ['id','name','username'], required: false }], order: [['createdAt', 'DESC']] });
...
router.get('/leads/filters', apiAuth, async (req, res) => {
  try {
    const user = req.session.user;
    const where = user.role === 'staff' ? { assigned_to: user.id } : {};
...
router.get('/leads/stats', apiAuth, async (req, res) => {
  try {
    ...
    res.json({ ok: true, total, assigned, unassigned, byStatus });
  } catch (e) { res.json({ ok: false, error: e.message }); }
});
...
router.put('/leads/:id', apiAuth, async (req, res) => {
  try {
    ...
    const user = req.session.user;
    if (user.role === 'staff' && lead.assigned_to !== user.id) return res.json({ ok: false, error: 'Not your lead' });
...
router.post('/leads/assign', apiAdmin, async (req, res) => {
  try {
    ...
    await Lead.update({ assigned_to: staff_id }, { where: { id: { [Op.in]: lead_ids } } });
...
router.post('/leads/:id/log', apiAuth, async (req, res) => {
  try {
    ...
    await CallLog.create({ lead_id: lead.id, staff_id: user.id, status, note });
    res.json({ ok: true });
  } catch (e) { res.json({ ok: false, error: e.message }); }
});
...
router.post('/import', apiAdmin, upload.single('file'), async (req, res) => {
  try {
    ...
    const leads = rows.map(row => ({ name: map.name ? String(row[map.name]||'').trim() : '', phone: map.phone ? String(row[map.phone]||'').trim() : '', email: map.email ? String(row[map.email]||'').trim() : '', city: map.city ? String(row[map.city]||'').trim() : '', state: map.state ? String(row[map.state]||'').trim() : '', category: map.category ? String(row[map.category]||category||'').trim() : (category||''), business_type: map.business_type ? String(row[map.business_type]||'').trim() : '', source: source||file.originalname, status: 'pending', extra: {...row} })).filter(l => l.name || l.phone);
